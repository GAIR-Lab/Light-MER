...
from my_affectgpt.models.ot_loss import OTProjector, sinkhorn_divergence, SWDProjector, sliced_wasserstein_loss
import config
...
class AffectGPT(Blip2Base):
    ...
    def set_teacher(self, teacher_model, ot_weight=1.0, ot_epsilon=0.1,
                    ...
                    use_swd=False, swd_n_projections=100, swd_p=2):
        """
        设置教师模型用于 OT 蒸馏。
        ...
        """
        self._teacher_container = [teacher_model]
        self.ot_weight_target = ot_weight
        self.ot_epsilon = ot_epsilon
        self.ot_num_iters = ot_num_iters
        self.ot_ramp_steps = ot_ramp_steps
        self.kl_weight = kl_weight
        self.kl_temperature = kl_temperature
        self.use_swd = use_swd
        self.swd_n_projections = swd_n_projections
        self.swd_p = swd_p
        ...
        student_llm_dim = self.llama_model.config.hidden_size
        teacher_llm_dim = teacher_model.llama_model.config.hidden_size

        if use_swd:
            self.ot_hidden_proj = SWDProjector(teacher_llm_dim, student_llm_dim)
        else:
            self.ot_hidden_proj = OTProjector(teacher_llm_dim, student_llm_dim, ot_common_dim)

        # Hard assertion: teacher_proj must be completely frozen
        for name, p in self.ot_hidden_proj.teacher_proj.named_parameters():
            assert not p.requires_grad, \
                f"FATAL: teacher_proj param '{name}' is trainable! This breaks OT geometry."
        teacher_proj_total = sum(p.numel() for p in self.ot_hidden_proj.teacher_proj.parameters())
        student_proj_trainable = sum(p.numel() for p in self.ot_hidden_proj.student_proj.parameters() if p.requires_grad)

        if use_swd:
            print(f'====== Teacher model set for SWD distillation ======')
            print(f'  Sliced Wasserstein Distance (NO bottleneck)')
            print(f'  Hidden-state SWD: student {student_llm_dim} → {student_llm_dim} (trainable: {student_proj_trainable:,} params)')
            print(f'                    teacher {teacher_llm_dim} → {student_llm_dim} (FROZEN: {teacher_proj_total:,} params, all requires_grad=False)')
            print(f'  n_projections={swd_n_projections}, p={swd_p}')
        else:
            print(f'====== Teacher model set for OT distillation ======')
            print(f'  Hidden-state OT: student {student_llm_dim} → {ot_common_dim} (trainable: {student_proj_trainable:,} params)')
            print(f'                   teacher {teacher_llm_dim} → {ot_common_dim} (FROZEN: {teacher_proj_total:,} params, all requires_grad=False)')
            print(f'  Cost function: cosine distance [0, 2]')
            print(f'  ot_weight={ot_weight}, epsilon={ot_epsilon}, iters={ot_num_iters}, ramp_steps={ot_ramp_steps}')
        print(f'  OT mask: CE-aligned shifted labels (positions that predict answer tokens)')
        print(f'  ot_weight={ot_weight}, ramp_steps={ot_ramp_steps}')
        print(f'  kl_weight={kl_weight}, kl_temperature={kl_temperature}')
    ...
    def forward(self, samples):
        ...
        if self.teacher is not None:
            global_step = int(samples.get('global_step', 0))
            current_ot_weight = self._get_ot_weight(global_step)

            # --- Teacher forward (frozen, no_grad) ---
            with torch.no_grad():
                teacher_dict = self.teacher._forward_with_hidden(samples)
                teacher_hidden = teacher_dict['hidden_states']
                teacher_logits = teacher_dict['logits']

            # --- Hidden-state OT (main distillation signal) ---
            student_hidden = outputs.hidden_states[-1]  # [b, seq, 1024]

            # CE-aligned mask: use the same shifted positions that CE loss optimizes.
            # CE internally does: loss_i = CE(logits[i], labels[i+1])
            # So the CE-relevant hidden states are at positions where labels[i+1] != IGNORE_INDEX.
            # hidden[..., :-1, :] with mask targets[..., 1:] != IGNORE gives exactly these positions.
            shift_targets = targets[..., 1:]  # [b, seq-1]
            answer_mask = (shift_targets != config.IGNORE_INDEX)  # [b, seq-1]
            student_hidden_ot = student_hidden[..., :-1, :]  # [b, seq-1, dim]
            teacher_hidden_ot = teacher_hidden[..., :-1, :]  # [b, seq-1, dim]

            # Safeguard: skip OT if too few answer tokens to avoid degenerate/noisy gradients
            min_answer_tokens = answer_mask.sum(dim=1).min().item()
            min_required = 2 if getattr(self, 'use_swd', False) else 4
            if min_answer_tokens >= min_required:
                if getattr(self, 'use_swd', False):
                    with torch.cuda.amp.autocast(enabled=False):
                        proj_teacher, proj_student = self.ot_hidden_proj(
                            teacher_hidden_ot.float(), student_hidden_ot.float())
                        ot_hidden_loss = sliced_wasserstein_loss(
                            proj_teacher, proj_student,
                            answer_mask,
                            n_projections=self.swd_n_projections,
                            p=self.swd_p)
                else:
                    with torch.cuda.amp.autocast(enabled=False):
                        proj_teacher, proj_student = self.ot_hidden_proj(
                            teacher_hidden_ot.float(), student_hidden_ot.float())
                        ot_hidden_loss = sinkhorn_divergence(
                            proj_teacher, proj_student,
                            epsilon=self.ot_epsilon,
                            num_iters=self.ot_num_iters,
                            mask_x=answer_mask, mask_y=answer_mask)
            else:
                ot_hidden_loss = torch.tensor(0.0, device=ce_loss.device)

            # --- Optional: KL loss on logits ---
            ...
            if self.kl_weight > 0:
                ...
                mask = (shift_labels != config.IGNORE_INDEX).float()
                T = self.kl_temperature
                with torch.cuda.amp.autocast(enabled=False):
                    s_log_probs = F.log_softmax(shift_student.float() / T, dim=-1)
                    t_probs = F.softmax(shift_teacher.float() / T, dim=-1)
                    kl_loss = F.kl_div(s_log_probs, t_probs, reduction='none')
            ...
            # --- Combined loss ---
            total_loss = (ce_loss
                         + current_ot_weight * ot_hidden_loss
                         + self.kl_weight * kl_loss)

            return {"loss": total_loss, "ce_loss": ce_loss,
                    "ot_hidden_loss": ot_hidden_loss,
                    "kl_loss": kl_loss, "ot_weight": current_ot_weight,
                    "logits": outputs.logits}
